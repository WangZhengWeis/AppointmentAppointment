...
namespace Xr.RtManager.Pages.scheduling
{
    public partial class ModifyNumberSourceEdit : Form
    {
        ...
        private List<SourceDataEntity> originalSourceDataList { get; set; }
        ...
        {
            List<SourceDataEntity> newSourceDataList = gcSourceData.DataSource as List<SourceDataEntity>;
            List<SourceDataEntity> subSourceDataList = new List<SourceDataEntity>();
            foreach (SourceDataEntity sd in originalSourceDataList)
            {
                foreach (SourceDataEntity newSd in newSourceDataList)
                {
                    if (sd.id.Equals(newSd.id))
                    {
                        if(!sd.numSite.Equals(newSd.numSite)||!sd.numOpen.Equals(newSd.numOpen)
                            || !sd.numClinic.Equals(newSd.numClinic))
                        {
                            subSourceDataList.Add(newSd);
                            break;
                        }
                    }
                }
            }
            String scheduSets = Newtonsoft.Json.JsonConvert.SerializeObject(subSourceDataList);
