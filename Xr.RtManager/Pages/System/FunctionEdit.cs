...
{
    public partial class FunctionEdit : Form
    {
        public FunctionEdit()
        {
            InitializeComponent();
        ...
        private void FunctionEdit_Load(object sender, EventArgs e)
        {
            ...
            dcFunction.DataType = typeof(FunctionEntity);
            if (function != null)
            {
                dcFunction.SetValue(function);
            }
            else
            {
        ...
        {
            if (!dcFunction.Validate())
            {
                return;
            }
            dcFunction.GetValue(function);
            String param = "?" + PackReflectionEntity<FunctionEntity>.GetEntityToRequestParameters(function);
            String url = AppContext.AppConfig.serverUrl + "sys/function/save" + param;
            cmd.ShowOpaqueLayer();
